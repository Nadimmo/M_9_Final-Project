...
const UpdateItem = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (res.data.success) {
      ...
      const result = await axiosSecure.patch(`/menu/${_id}`, menuItem);
      if (result.data.modifiedCount > 0) {
        ...
        Swal.fire({
          position: "top-end",
          icon: "success",
          title: `${data.name} update this recipe.`,
  ...
  return (
    <div>
      <Title title={"What's a new?"} short={"update an item"}></Title>
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-10 my-10">
        ...
        <button className="btn bg-[#D1A054] mt-5">Update Recipe Details</button>
      </form>
    </div>
  );
};
